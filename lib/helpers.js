...
/**
 * Gets an array from a string of comma separated values
 * @param  {String} string the string to convert
 * @return {Array} and array of values in the string
 */
...
getMainBranchFields = function () {
    ...
    var crmTaskNumber = $('#crmTaskNumber').val().length > 0 &&
        getArrayFromString($('#crmTaskNumber').val()).map(Number) || null;
    var sprints = $('#sprints').val().length > 0 &&
        getArrayFromString($('#sprints').val()).map(Number) || null;
